...
export const useProfile = (id?: string, predicate?: string) => {
    ...
    const { data: followings, isLoading: loadingFollowings } = useQuery<Profile[]>({
        ...
        queryFn: async () => {
            const response = await agent.get<Profile[]>(`/profiles/${id}/follow-list`, {
                params: { predicate }
            });
            return response.data;
        },
    ...
    const { data: userActividades, isLoading: loadingUserActividades } = useQuery<Actividad[]>({
        queryKey: ['userActividades', id, filter],
        queryFn: async () => {
            const response = await agent.get<Actividad[]>(`/profiles/${id}/actividades`, {
                params: { filter }
            });
            return response.data;
        },
