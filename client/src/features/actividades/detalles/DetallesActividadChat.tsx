...
export default observer(function DetallesActividadChat() {
    const { id } = useParams();
    ...
    const handleAddComment = async (data: FieldValues ) => {
        try {
            await commentStore.hubConnection?.invoke("SendComment", { actividadId: id, body: data.body });
            reset();
        } catch (error) {
                        ...
                        <Box key={comment.id} sx={{ display: 'flex', my: 2 }}>
                            ...
                            <Box display='flex' flexDirection='column'>
                                <Box display='flex' alignItems='center' gap={3}>
                                    <Typography component={Link} to={`/perfiles/${comment.userId}`} variant="subtitle1" sx={{ fontWeight: 'bold', textDecoration: 'none' }}>
                                        {comment.displayName}
                                    </Typography>
